...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	//from locale file converting to ascii string 
	...
	for (size_t s = 0; s < shapes.size(); s++)
	{
		...
		for (size_t f = 0; shapes[s].mesh.num_face_vertices.size(); f++)
		{
	...
	m_vertex_buffer = GraphicsEngine::get()->getRenderSystem()->createVertexBuffer(
		&list_vertices[0], sizeof(VertexMesh), list_vertices.size(), shader_byte_code, (UINT)size_shader);

	m_index_buffer = GraphicsEngine::get()->getRenderSystem()->createIndexBuffer(&list_indices[0],
